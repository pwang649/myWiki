...
1. $\min_{u(\cdot)} \max_{d(\cdot)} J(\cdot) \rightarrow$ some objective that combines distance to the goals while avoiding collision.
    ...
    $u(\cdot)$ will declare its trajectory first. $d(\cdot)$ will then try to interscept this trajectory. $\rightarrow$ *Overly pessimistic*
...
Above formulations are not suitable to describe most practical systems. Typically, at time $t$, we only have information upto time $t$. Such strategies are called non-anticipative strategies.
...
HJI equation looks very similar to HJB equation with min operator replaces by min-max operator.

The tools that are available to solve HJB equation can also be used to solve the HJI equation (at least most of them can be used).
